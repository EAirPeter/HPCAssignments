...
//
{
  ...
  for ( i = 2; i < n; i++ ) // was i <= n, changed to 1 < n
  {
    x[i] = x[i-1] + x[i-2]; // detected invalid write on this line
    cout << "  " << i << "  " << x[i] << "\n"; // detected invalid read on
                                               // this line
  }

  // delete [] x; // detected mismatched free() / delete / delete []
  free(x); // changed to free(x)
